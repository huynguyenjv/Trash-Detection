...
import os
import matplotlib
# Thiết lập backend phù hợp cho Linux
if 'DISPLAY' in os.environ:
    matplotlib.use('TkAgg')  # Hoặc 'Qt5Agg' nếu có PyQt5
else:
    matplotlib.use('Agg')    # Cho server không có GUI
...
def demo_interactive_map():
    ...
    # Kiểm tra backend và hiển thị phù hợp
    backend = matplotlib.get_backend()
    print(f"🖥️  Using matplotlib backend: {backend}")
    
    if backend == 'Agg':
        print("⚠️  GUI backend not available. Saving plot as image instead.")
        fig.savefig('interactive_map.png', dpi=150, bbox_inches='tight')
        print("💾 Map saved as 'interactive_map.png'")
    else:
        try:
            plt.show()
        except Exception as e:
            print(f"⚠️  GUI display failed: {e}")
            print("💾 Saving plot as image instead...")
            fig.savefig('interactive_map.png', dpi=150, bbox_inches='tight')
            print("💾 Map saved as 'interactive_map.png'")
