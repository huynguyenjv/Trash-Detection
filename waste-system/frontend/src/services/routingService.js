...
export async function findNearestBin({
  ...
  try {
    ...
    // Decode polyline if available (Goong Maps returns encoded polyline)
    if (data.route?.polyline && data.method === 'goong_maps') {
      const coords = await decodePolyline(data.route.polyline);
      data.route.coordinates = coords;
    }
    
    // For straight_line fallback, create simple 2-point path
    if (data.method === 'straight_line' && data.nearest_bin && !data.route?.coordinates) {
      data.route = data.route || {};
      data.route.coordinates = [
        [latitude, longitude],
        [data.nearest_bin.latitude, data.nearest_bin.longitude]
      ];
      data.route.distance_km = data.distance_km || 0;
      data.route.duration_minutes = data.distance_km ? data.distance_km * 12 : 5; // ~5 km/h walking
    }
...
export async function optimizeRoute({
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/routing/optimize-route`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
export async function decodePolyline(encoded) {
  try {
    const response = await fetch(
      `${API_BASE_URL}/routing/decode-polyline?encoded=${encodeURIComponent(encoded)}`
    );
